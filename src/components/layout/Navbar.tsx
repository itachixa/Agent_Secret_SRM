...
import {
  ...
  Users, Calendar, BookOpen, Bell, User,
  Menu, X, ChevronDown, LogOut, Settings
} from 'lucide-react';
import { useState, useEffect, useRef } from 'react';

const mockUser = { avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=current', name: 'User' };
          ...
          <div className="w-full max-w-7xl mx-auto px-6 flex items-center justify-between">
            ...
            <div className="flex items-center gap-0.5">
              {mainNavItems.map(({ href, icon: Icon, key }) => {
                ...
                return (
                  <Link key={href} href={href} className={`nav-link ${isActive ? 'nav-link-active' : 'nav-link-inactive'}`}>
                    <Icon size={16} />
            ...
            <div className="flex items-center gap-2">
              <LanguageToggle />
<Link href="/messages" className={`relative p-2.5 rounded-xl transition-all duration-200 ${pathname === '/messages' ? 'text-togo-green bg-togo-green/15' : 'text-gray-400 hover:text-white hover:bg-white/[0.04]'}`}>
              <MessageSquare size={20} />
            </Link>
              <Link href="/notifications" className={`relative p-2.5 rounded-xl transition-all duration-200 ${pathname === '/notifications' ? 'text-togo-green bg-togo-green/15' : 'text-gray-400 hover:text-white hover:bg-white/[0.04]'}`}>
                <Bell size={20} />
                <span className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-togo-yellow rounded-full border-2 border-togo-dark" />
              </Link>

              <div className="relative" ref={profileRef}>
                <button onClick={() => setProfileOpen(!profileOpen)} className="flex items-center gap-2 p-1.5 rounded-xl hover:bg-white/[0.04] transition-colors ml-1">
                  <img src={mockUser.avatar} alt={mockUser.name} className="w-8 h-8 rounded-full ring-2 ring-togo-green/40" />
                  ...
                  <div className="absolute right-0 top-full mt-2 w-60 bg-togo-dark/95 backdrop-blur-xl border border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
                    <div className="p-4 border-b border-white/[0.06]">
                      <div className="flex items-center gap-3">
                        <img src={mockUser.avatar} alt={mockUser.name} className="w-10 h-10 rounded-full ring-2 ring-togo-green/30" />
                        <p className="font-semibold text-white text-sm truncate">{mockUser.name}</p>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-40">
          ...
          <div className="absolute right-0 top-14 bottom-16 w-72 bg-togo-dark/95 backdrop-blur-xl border-l border-white/[0.06] animate-slide-in-right overflow-y-auto">
            <div className="p-4 border-b border-white/[0.06]">
              <div className="flex items-center gap-3">
                <img src={mockUser.avatar} alt={mockUser.name} className="w-12 h-12 rounded-full ring-2 ring-togo-green/40" />
                <p className="font-semibold text-white truncate">{mockUser.name}</p>
                <p className="text-sm text-gray-400 truncate">Togo-India</p>
              </div>
            </div>
            <div className="p-3">
              {mainNavItems.map(({ href, icon: Icon, key }) => {
                const isActive = pathname === href || (href !== '/' && pathname.startsWith(href));
                return (
                  <Link key={href} href={href} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${isActive ? 'bg-togo-green/15 text-togo-green' : 'text-gray-400 hover:text-white hover:bg-white/[0.04]'}`}>
                    <Icon size={20} />
                    {t(key)}
                  </Link>
                );
              ...
              <div className="border-t border-white/[0.06] mt-2 pt-2">
                <Link href="/messages" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-400 hover:text-white hover:bg-white/[0.04]">
                  <MessageSquare size={20} />
                  {t('nav.messages')}
                </Link>
                <Link href="/profile" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-400 hover:text-white hover:bg-white/[0.04]">
                  <User size={20} />
                  {t('nav.profile')}
                </Link>
              </div>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 h-16 bg-togo-dark/90 backdrop-blur-xl border-t border-white/[0.06]">
        <div className="h-full flex items-center justify-around px-2">
          {bottomNavItems.map(({ href, icon: Icon, key }) => {
            ...
            return (
              <Link key={href} href={href} className={`flex flex-col items-center gap-0.5 p-2 rounded-xl transition-all min-w-[56px] ${isActive ? 'text-togo-green' : 'text-gray-500'}`}>
                <Icon size={22} />
