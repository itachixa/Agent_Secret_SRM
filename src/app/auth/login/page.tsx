      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400 mt-2 text-sm">
            {lang === 'fr' ? 'Connectez-vous à votre compte' : 'Sign in to your account'}
          </p>
        </div>
        ...
        <div className="glass-card p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <a href="#" className="text-xs text-togo-green hover:underline">{t('auth.forgot')}</a>
              </div>
              <div className="relative">
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" className="w-4 h-4 rounded border-white/20 bg-white/5 text-togo-green focus:ring-togo-green/50" />
              <span className="text-sm text-gray-400">{lang === 'fr' ? 'Se souvenir de moi' : 'Remember me'}</span>
            </label>

            <button
              type="submit"
              disabled={isLoading}
              className="btn-primary w-full flex items-center justify-center gap-2 py-3.5 text-[15px] disabled:opacity-50"
            >
              {isLoading ? (
          ...
          <div className="flex items-center gap-4 my-7">
            ...
            <span className="text-xs text-gray-500 uppercase tracking-wider">{t('auth.loginWith')}</span>
            <div className="flex-1 h-px bg-white/[0.06]" />
          </div>

          {/* Social Login */}
          <div className="grid grid-cols-2 gap-3">
            <button className="btn-secondary flex items-center justify-center gap-2.5 py-3 text-sm">
        ...
        <p className="text-center text-gray-400 mt-8 text-sm">
          {t('auth.noAccount')}{' '}
          <Link href="/auth/register" className="text-togo-green hover:underline font-medium">
            {t('auth.register')}
          </Link>
        </p>
      </div>
