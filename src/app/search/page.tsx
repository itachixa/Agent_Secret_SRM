...
export default function SearchPage() {
  ...
  const { data: users, loading, error } = useProfiles({ query, limit: 100 });
    ...
    <div className="page-container">
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setShowMentorsOnly(!showMentorsOnly)} className={`btn-outline flex items-center gap-2 text-sm ${showMentorsOnly ? 'bg-purple-500/15 border-purple-500/30 text-purple-400' : ''}`}>
      ...
      <div className="glass-card p-4 mb-6">
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t('search.placeholder')} className="input-field pl-11" />
          </div>
          <button onClick={() => setShowFilters(!showFilters)} className={`btn-secondary flex items-center gap-2 ${showFilters ? 'bg-togo-green/15 text-togo-green border-togo-green/20' : ''}`}>
          ...
          <Link key={user.id} href={`/profile/${user.id}`} className="glass-card-hover p-5 group">
            <div className="flex items-start gap-4">
              <div className="relative flex-shrink-0">
                <img src={user.avatar_url || 'https://api.dicebear.com/7.x/avataaars/svg?seed=' + user.id} alt={user.full_name} className={`${viewMode === 'grid' ? 'w-16 h-16' : 'w-14 h-14'} rounded-full ring-2 ring-white/[0.08] group-hover:ring-togo-green/40 transition-all`} />
                {user.is_online && <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-emerald-500 rounded-full border-2 border-togo-dark" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="font-semibold text-white group-hover:text-togo-green transition-colors">{user.full_name}</h3>
                  <span className={`badge-${user.badge}`}>{t(`profile.badge.${user.badge}`)}</span>
                </div>
                <p className="text-sm text-gray-400 truncate mt-0.5">{user.field}</p>
